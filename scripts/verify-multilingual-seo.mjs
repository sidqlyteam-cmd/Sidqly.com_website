...
function checkHtmlFile(target) {
  ...
  if (!exists) {
    return {
      pass: false,
      error: `Static file ${target.file} does not exist in dist/`,
    };
  }
  ...
  const hasHreflangAr =
    content.includes('hrefLang="ar"') ||
    content.includes('hreflang="ar"');

  const hasHreflangUr =
    content.includes('hrefLang="ur"') ||
    content.includes('hreflang="ur"');

  const hasHreflangFr =
    content.includes('hrefLang="fr"') ||
    content.includes('hreflang="fr"');
...
const pagesSitemap = fs.readFileSync(
  path.join(publicDir, 'sitemap-pages.xml'),
  'utf8'
);

const locationsSitemap = fs.readFileSync(
...
testUrls.forEach((t) => {
  ...
  console.log(
    `[Sitemap] ${fullLoc}: ${inSitemap ? 'PASS' : 'FAIL'}`
  );
...
testUrls.forEach((t) => {
  ...
  console.log(
    `[HTML Static Check] ${t.url} (${t.lang}): ${
      res.pass ? 'PASS' : 'FAIL'
    }`
  );
...
if (overallPass) {
  console.log(
    'ALL 17 PRODUCTION MULTILINGUAL SEO CHECKS PASSED PERFECTLY!'
  );
  process.exit(0);
} else {
